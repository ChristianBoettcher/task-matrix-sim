...
char * tasks[MAX];
//counters and such for put and get methods
int fill_ptr = 0;
int use_ptr = 0;
int count = 0;

//need *tasks[0] to access first element
//we can define as char * tasks[MAX] to access with tasks[0]
...
void sleepms(int milliseconds) 
{
	...
	struct timespec *req = &required;
	struct timespec *rem = &remember;


	long nano = milliseconds * 1000000;
	req->tv_nsec = nano;
	printf("%lu\n", nano);
	nanosleep(req,rem);
	return;
...
//arg to be passed in will result in the ms_sleep duration;
int put(char * input){
	if(count >= MAX){
		return -1; //go to sleep 
	}else{  
	// place in buffer, adv the fill ptr, count++
	// if the consumer threads are asleep, wake them up...perhaps only one
	// don't forget mutex-ing
		char * newtask = malloc(sizeof(char)* BUFFSIZ);
		strcpy(newtask, input);
		tasks[fill_ptr] = newtask;
...
char * get(){
	char * output = NULL;
	if(count == 0){
		return output; // go to sleep
	} else {
	// extract task from buffer, adv the use ptr, count--
	// if the producer thread is asleep, wake it up
	// don't forget mutex-ing
		output = malloc(sizeof(char) * BUFFSIZ);
		strcpy(output, tasks[use_ptr]);
		free(tasks[use_ptr]);
...
void *readtasks(void *arg)
{
    ...
    while (1)
    {
		...
		strcpy(copy, buffer);
	      ...
	      //-------instantiated cond_t "empty" and "full" as well as mutex_t "mutex"------
	      //-------------------------code from the book (pg 352)------------------------
...
//------------------------------------------------------------------
    ...
    //once bounded buffer is set, no need to malloc, we just call the buffer 
    //using the get routine, this will cause a memory leak if left in
